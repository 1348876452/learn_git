...
void topTwoIdx(uint64_t* inArray, uint64_t inArraySize, uint8_t* outIdxArray, uint64_t* outValArray) {
	...
	printf("outValArray[1] and outValArray[0]is %d,%d\n",outValArray[1],outValArray[0]);
}
...
int main(void) {
    ...
    uint64_t start, diff, passInIdx, randIdx,observe_0=0,observe_1=0;
    uint8_t dummy = 0;
    ...
    for(uint64_t len = 0; len < SECRET_SZ; ++len) {
		...
		passInIdx = randIdx ^ (passInIdx & (attackIdx ^ randIdx)); // select randIdx or attackIdx   
                if(observe_1 != passInIdx)
		{
		observe_1=passInIdx;
		printf("passInIdx is %x attackIdx is %x\n",passInIdx,attackIdx);
